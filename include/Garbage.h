...
struct Garbage_Env {
    ...
    /*
     * OBSOLETE: use a `Qntan_Scheduler` instead. */
// OBSOLETE void (*setTimeoutMs)(
// OBSOLETE     struct Garbage_Env**, struct Qntan_Executor**, int delayMs,
// OBSOLETE     void(*fn)(int eno, Qntan_Cls), Qntan_Cls );
};
...
struct Garbage_AppCtx {
	/**/
	struct Qntan_Mallocator** (*getMallocator)( struct Garbage_AppCtx** );
	/**/
	struct Qntan_EvLoop** (*getEvLoop)( struct Garbage_AppCtx** );
	/**/
	struct Garbage_Env** (*getEnvLegacy)( struct Garbage_AppCtx** );
	/* DEPRECATED! Use `getEvLoop()` instead.
	 */
	struct Qntan_Executor** (*getExecutorCpu)( struct Garbage_AppCtx** );
	/**/
	struct Qntan_Executor** (*getExecutorIo)( struct Garbage_AppCtx** );
	/**/
	struct Qntan_Scheduler** (*getScheduler)( struct Garbage_AppCtx** );
	/**/
	struct Qntan_LoggerFactory** (*getLoggerFactory)( struct Garbage_AppCtx** );
	/* (optional) loggerFactory to use, to create requested loggers. If
	 * not given, AppCtx SHOULD fallback to `getLoggerDefault()`.
	 */
	struct Qntan_Logger** (*getLoggerDefault)( struct Garbage_AppCtx** );
	/* (optional) logger to use as the fallback logger. HINT: if
	 * `getLoggerFactory()` is set, this field here SHOULD be ignored by
	 * AppCtx.
	 */
	struct Qntan_FileSystem** (*getFilesystem)( struct Garbage_AppCtx** );
	/**/
	struct Qntan_IoMux** (*getIoMux)( struct Garbage_AppCtx** );
	/**/
	struct Qntan_Networker** (*getNetworker)( struct Garbage_AppCtx** );
	/**/
};
...
struct Garbage_GagaFont {
	/**/
	void (*draw)( struct Garbage_GagaFont**, struct Qntan_GfxRenderer**,
		float x, float y, float fontSize, unsigned colorRGBA8888,
		char const*str, int str_len );
	/* `y` is baseline of the font where to start the writing.
	 * `str` the text to write.
	 * `str_len` how many OCTETS `str` is in length.
	 */
};
...
struct Garbage_SwsquotCsvParser_Transaction {
	int date_len     ; char const *date;
	int auftragNr_len; char const *auftragNr;
	int type_len     ; char const *type;
	int symbol_len   ; char const *symbol;
	int isin_len     ; char const *isin;
	int count_len    ; char const *count;
	int pricePcs_len ; char const *pricePcs;
	int cost_len     ; char const *cost;
	/* TODO "Aufgelaufene Zinsen" */
	int netAmt_len   ; char const *netAmt;
	int balance_len  ; char const *balance;
	int ccy_len      ; char const *ccy;
};

struct Garbage_SwsquotCsvParser_Mentor {
	/**/
	void (*onRecord)(
		struct Garbage_SwsquotCsvParser_Mentor**, struct Garbage_SwsquotCsvParser_Transaction*,
		void(*onDone)(Qntan_Cls), Qntan_Cls cls);
	/**/
};
...
struct Garbage_SwsquotCsvParser {
	/**/
	void (*write)(
		struct Garbage_SwsquotCsvParser**, void*buf, int buf_len, int flgs,
		void(*onDone)(int,Qntan_Cls), Qntan_Cls );
	/* @param flgs
	 *      `flgs & 4` set means this is the last write call, then the stream
	 *      has ended. All other bits MUST be set to zero!
	 */
};
